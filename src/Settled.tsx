...
function mapToDb(debt, userId) {
  return {
    ...
    rate_changes: debt.rateChanges || [],
    rate_history: debt.rateHistory || [],
...
function mapFromDb(row) {
  return {
    ...
    useAutoMin: row.use_auto_min,
    remainingMonths: row.remaining_months,
    termMonths: row.term_months,
    startDate: row.start_date,
    paymentDueDay: row.payment_due_day,
    payoffGoalMonths: row.payoff_goal_months,
...
function calcMinPayment(balance, apr, type, remainingMonths) {
  const b = Number(balance),
    rate = Number(apr) / 100 / 12;
  if (!b || b <= 0) return 0;
  if (['Credit Card', 'Overdraft', 'Buy Now Pay Later'].includes(type)) {
    if (!apr || Number(apr) === 0)
      return Math.min(b, b < 25 ? b : Math.max(25, b * 0.01));
    return Math.min(b, b < 25 ? b : Math.max(25, b * 0.02));
  }
  const n =
    Number(remainingMonths) > 0
      ? Number(remainingMonths)
      : { Mortgage: 300, 'Car Loan': 60 }[type] || 60;
  if (!apr || Number(apr) === 0) return Math.min(b, b / n);
  return Math.min(
    b,
    (b * rate * Math.pow(1 + rate, n)) / (Math.pow(1 + rate, n) - 1)
  );
}
...
function simulatePayoff(balance, currentApr, monthlyPayment, rateChanges = []) {
  ...
  const pending = (rateChanges || [])
    .filter((rc) => !rc.applied && rc.date)
    .map((rc) => {
      ...
      return {
        ...
        rate: Number(rc.newRate),
      };
    })
    .sort((a, z) => a.offset - z.offset);
  let apr = Number(currentApr),
    interestPaid = 0,
    month = 0;
  while (b > 0 && month < 1200) {
    for (const rc of pending) {
      if (rc.offset === month) apr = rc.rate;
    }
    const rate = apr / 100 / 12,
      interest = b * rate;
    if (rate > 0 && pmt <= interest)
  ...
  return month >= 1200
    ? { months: Infinity, interest: Infinity }
    : { months: month, interest: interestPaid };
}
...
function simulateBudgetPayoff(debts, strategy, monthlyExtra) {
  ...
  while (pool.some((d) => d.balance > 0) && month < MAX) {
    ...
    for (const p of active) {
      ...
      const payment = Math.min(d.balance, pot);
      d.balance = Math.max(0, d.balance - payment);
      pot -= payment;
    }
    const cleared = [];
    for (const d of pool) {
      if (d.balance < 0.01 && !d.clearedAt) {
        d.balance = 0;
        d.clearedAt = month;
        extra += d.minPayment;
        cleared.push(d);
    ...
    if (cleared.length > 0)
      milestones.push({
        month,
        date: dateInMonths(month),
        cleared,
        newExtra: extra,
        remaining: pool.filter((d) => d.balance > 0).length,
      });
  }
...
function buildPaymentTimeline(debts) {
  ...
  const enriched = debts.map((d) => {
    const pmt = d.payoffGoalMonths
      ? calcGoalPayment(d.balance, d.interestRate, d.payoffGoalMonths) ||
        Number(d.minimumPayment)
      : Number(d.minimumPayment);
    const { months } = simulatePayoff(
      d.balance,
      d.interestRate,
      pmt,
      d.rateChanges
    ...
    return {
      ...d,
      monthlyPayment: pmt,
      clearsAt: isFinite(months) ? months : null,
    };
  });
  const startTotal = enriched.reduce((s, d) => s + d.monthlyPayment, 0);
  const byMonth = {};
  for (const d of enriched) {
    if (d.clearsAt === null) continue;
    if (!byMonth[d.clearsAt]) byMonth[d.clearsAt] = [];
    byMonth[d.clearsAt].push(d);
  }
  const milestones = [
    {
      month: 0,
    ...
    >
      <div
        style={{
          textAlign: 'center',
          padding: 32,
          animation: 'celebIn 0.4s cubic-bezier(.34,1.56,.64,1)',
        }}
      >
        <div style={{ fontSize: 72, marginBottom: 16, display: 'block' }}>
          {emoji}
        </div>
        <div
          style={{
        ...
        {subtitle && (
          <div style={{ fontSize: 15, color: '#94a3b8' }}>{subtitle}</div>
        )}
...
function ShareCardModal({
  ...
  function downloadCard() {
    ...
    ctx.fillText(
      `${progress}% of my total debt cleared — and counting.`,
      40,
      155
    );
    // Progress bar bg
    ...
    ctx.beginPath();
    ctx.roundRect(40, 185, 520, 14, 7);
    ctx.fill();
    // Progress bar fill
    ctx.fillStyle = grad;
    ctx.beginPath();
    ctx.roundRect(40, 185, Math.max(10, 520 * (progress / 100)), 14, 7);
    ctx.fill();
    // Stats
    ...
    ctx.fillText(
      `${debts.length} debt${
        debts.length !== 1 ? 's' : ''
      } tracked · ${formatCurrency(totalDebt)} remaining`,
      40,
      224
    );
    // Footer
    ...
    >
      ...
      >
        ...
        <div style={{ fontSize: 13, color: '#475569', marginBottom: 20 }}>
          Show the world you're winning with your debt.
        </div>
        <div
          style={{
            background: 'linear-gradient(135deg,#0d0d17,#12122a)',
            border: '1px solid #1e1e3a',
            borderRadius: 16,
            padding: 24,
            marginBottom: 16,
        ...
        >
          ...
          >
            I've paid off {formatCurrency(totalPaid)}
          </div>
...
function FreedomScreen({ totalPaid, startDate, onDismiss }) {
  ...
  const months = startDate
    ? Math.round((new Date() - new Date(startDate)) / 2628000000)
    : null;
  ...
  function downloadCert() {
    ...
    ctx.font = '18px sans-serif';
    ctx.fillText(
      'in total debt' + (months ? ` over ${months} months` : ''),
      400,
      310
    );
    ctx.fillStyle = '#475569';
      ...
      <button
        ...
        onClick={() => {
          if (navigator.share) {
            navigator.share({
              title: "I'm debt free!",
              text: `I just paid off ${formatCurrency(
                totalPaid
              )} of debt using Settled!`,
              url: 'https://settledapp.co.uk',
            });
          } else {
            navigator.clipboard.writeText(
              `I just paid off ${formatCurrency(
                totalPaid
              )} of debt using Settled! settledapp.co.uk`
            );
          }
        }}
      >
    ...
    >
      ...
      >
        ...
        <div
          style={{
            background: isPremium
              ? 'linear-gradient(135deg,#6366f115,#a78bfa12)'
              : '#0d0d17',
            border: isPremium ? '1px solid #6366f140' : '1px solid #1e1e3a',
            borderRadius: 12,
            padding: 16,
            marginBottom: 16,
          }}
        >
          ...
          <div style={{ fontSize: 15, color: '#e2e8f0', fontWeight: 600 }}>
            {isPremium ? '✦ Full Access — Lifetime' : 'Free Plan'}
          </div>
          {!isPremium && (
            <div style={{ fontSize: 12, color: '#475569', marginTop: 4 }}>
              Upgrade to unlock Pay Faster, Goals, and more
            </div>
          )}
        </div>
        {msg && (
...
function SliderSection({
  ...
  const baseMonths =
    debtFreeProjection && !debtFreeProjection.infinite
      ? debtFreeProjection.months
      : null;
  const baseInterest = debts.reduce((s, d) => {
    const { interest } = simulatePayoff(
      d.balance,
      d.interestRate,
      d.minimumPayment,
      d.rateChanges
    );
    return s + (isFinite(interest) ? interest : 0);
  }, 0);
  const boosted =
    extra > 0 ? simulateBudgetPayoff(debts, strategy, extra) : null;
  const boostedMonths = boosted && !boosted.shortfall ? boosted.months : null;
  const monthsSaved =
    baseMonths && boostedMonths ? baseMonths - boostedMonths : null;
  const interestSaved =
    boosted && !boosted.shortfall ? baseInterest - boosted.totalInterest : null;
    ...
    <div>
      ...
      >
        ...
        >
          ...
          >
            £{extra}
            <span style={{ fontSize: 13, color: '#475569', fontWeight: 400 }}>
              /mo
            </span>
          </div>
        </div>
          ...
          {[25, 50, 100, 200].map((v) => (
            <button
              ...
              style={{
                ...
                fontSize: 12,
                fontWeight: 600,
                background: extra === v ? '#6366f1' : '#1e1e3a',
                color: extra === v ? '#fff' : '#64748b',
                transition: 'all 0.2s',
              }}
              onClick={() => setExtra(v)}
            >
      ...
      {extra === 0 && baseMonths && (
        ...
        >
          ...
          >
            {debtFreeProjection?.words} to debt-free
          </div>
          <div style={{ fontSize: 13, color: '#64748b' }}>
            Move the slider above to see how extra payments help
          </div>
        </div>
      )}
      ...
      {extra > 0 && monthsSaved !== null && monthsSaved > 0 && (
        <div
          style={{
            background: 'linear-gradient(135deg,#34d39915,#6366f112)',
            border: '1px solid #34d39940',
            borderRadius: 16,
            padding: 18,
            marginBottom: 16,
        ...
        >
          ...
          >
            {monthsToWords(boostedMonths)} to debt-free
          </div>
          <div style={{ display: 'flex', gap: 20 }}>
            <div>
              <div style={{ fontSize: 11, color: '#64748b' }}>Time saved</div>
              <div style={{ fontSize: 18, fontWeight: 700, color: '#34d399' }}>
                {monthsToWords(monthsSaved)}
              </div>
            </div>
            {interestSaved > 0 && (
              <div>
      ...
      {extra > 0 && priorityName && (
        ...
        >
          ...
          <div style={{ fontSize: 13, color: '#e2e8f0', lineHeight: 1.5 }}>
            Your extra £{extra}/mo goes to <strong>{priorityName}</strong> first
            ({priorityRate}% APR —{' '}
            {strategy === 'avalanche' ? 'highest rate' : 'smallest balance'}).
            Once cleared, that money automatically rolls into the next debt.
          </div>
        </div>
      )}
      ...
      {boosted &&
        !boosted.shortfall &&
        boosted.milestones?.length > 0 &&
        (() => {
          const allMilestones = [
            { month: 0, date: 'Now', cleared: [], remaining: debts.length },
            ...boosted.milestones,
          ];
          let clearedMinsTotal = 0;
          const milestonesWithTotals = allMilestones.map((m, i) => {
            if (i > 0)
              clearedMinsTotal += m.cleared.reduce(
                (s, d) => s + d.minPayment,
                0
              );
            return {
              ...m,
              monthlyTotal: totalMinPayment - clearedMinsTotal + extra,
            };
          });
          ...
          return (
            <div style={{ marginTop: 4 }}>
              ...
              <div style={{ fontSize: 12, color: '#475569', marginBottom: 16 }}>
                Watch your total monthly payment drop as each debt clears
              </div>
              {milestonesWithTotals.map((m, i, arr) => {
                ...
                const barWidth =
                  startTotal > 0 ? (m.monthlyTotal / startTotal) * 100 : 0;
                return (
                  <div
                    key={i}
                    style={{
                      ...
                      >
                        <div>
                          <div
                            style={{
                              fontSize: 13,
                              fontWeight: 700,
                              color: isFirst ? '#a78bfa' : '#e2e8f0',
                            }}
                          >
                            {isFirst ? 'Today' : m.date}
                          </div>
                          {m.cleared &&
                            m.cleared.map((d) => (
                              <div
                        ...
                        <div style={{ textAlign: 'right' }}>
                          ...
                          >
                            {isLast && m.remaining === 0
                              ? '£0/mo'
                              : formatCurrency(m.monthlyTotal) + '/mo'}
                          </div>
                          {drop > 0 && (
                            <div
                              style={{
...
export default function DebtTracker() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (settingsRes.data) {
        ...
        setBudgetIncome(settingsRes.data.budget_income || 0);
        setBudgetExpenses(settingsRes.data.budget_expenses || 0);
      }
  ...
  useEffect(() => {
    ...
    async function applyDueRateChanges() {
      ...
      setDebts((prev) => {
        const next = prev.map((debt) => {
          ...
          const newDebt = {
            ...
            rateChanges: debt.rateChanges.map((rc) =>
              rc.date === latest.date ? { ...rc, applied: true } : rc
            ),
            rateHistory: [
              ...(debt.rateHistory || []),
              { date: today, rate: newRate },
            ],
          };
      ...
      for (const debt of updated)
        await supabase
          .from('debts')
          .update(mapToDb(debt, session?.user?.id))
          .eq('id', debt.id);
    }
    applyDueRateChanges();
    ...
    async function autoApplyDuePayments() {
      ...
      setDebts((prev) => {
        const next = prev.map((debt) => {
          ...
          const newRemaining = debt.remainingMonths
            ? Math.max(1, debt.remainingMonths - 1)
            : null;
          const newMin =
            debt.useAutoMin !== false
              ? calcMinPayment(
      ...
      if (toUpdate.length > 0) {
        setCelebration({
          emoji: '✅',
          title: `${toUpdate.length} minimum payment${
            toUpdate.length !== 1 ? 's' : ''
          } applied`,
          subtitle: `Auto-applied for: ${toUpdate
            .map((d) => d.name)
            .join(', ')}. Log extra payments manually.`,
        });
      }
    }
  ...
  const isRevolving = (t) =>
    ['Credit Card', 'Overdraft', 'Buy Now Pay Later'].includes(t);
  const autoMin = useMemo(
    () =>
      calcMinPayment(
        ...
        form.interestRate,
        form.type,
        form.totalTermMonths || null
      ),
    [form.balance, form.interestRate, form.type, form.totalTermMonths]
  );

  const activeDebts = useMemo(
    () => debts.filter((d) => Number(d.balance) > 0),
    [debts]
  );
  ...
  const totalDebt = useMemo(
    () => activeDebts.reduce((s, d) => s + Number(d.balance), 0),
    [activeDebts]
  );
  const totalOriginal = useMemo(
    () => debts.reduce((s, d) => s + Number(d.originalBalance || d.balance), 0),
    [debts]
  );
  ...
  const totalMinPayment = useMemo(
    () => activeDebts.reduce((s, d) => s + Number(d.minimumPayment || 0), 0),
    [activeDebts]
  );

  // Freedom check
  useEffect(() => {
    if (debts.length > 0 && activeDebts.length === 0 && !freedomDismissed)
      setShowFreedom(true);
  ...
  const debtFreeProjection = useMemo(() => {
    ...
    for (const d of activeDebts) {
      ...
      if (!isFinite(months)) {
        const mi = Number(d.balance) * (Number(d.interestRate) / 100 / 12),
          tm = d.remainingMonths || 36,
          rate = Number(d.interestRate) / 100 / 12;
        const rec =
          rate === 0
            ? Number(d.balance) / tm
            : (Number(d.balance) * rate * Math.pow(1 + rate, tm)) /
              (Math.pow(1 + rate, tm) - 1);
        problemDebts.push({
          ...d,
          monthlyInterest: mi,
          recommended: rec,
    ...
    return {
      months: maxMonths,
      date: dateInMonths(maxMonths),
      words: monthsToWords(maxMonths),
    };
  }, [activeDebts]);
  ...
  const goalProjection = useMemo(() => {
    if (
      ...
      !activeDebts.some((d) => d.payoffGoalMonths)
    )
      return null;
    let maxMonths = 0,
      totalMonthly = 0;
    for (const d of activeDebts) {
      const goalPmt = d.payoffGoalMonths
        ? calcGoalPayment(d.balance, d.interestRate, d.payoffGoalMonths)
        : Number(d.minimumPayment);
  ...
  const budgetMonthlyExtra =
    budgetDisposable > 0 ? Math.max(0, budgetDisposable - totalMinPayment) : 0;
  const budgetProjection = useMemo(() => {
    if (!budgetIncome || budgetDisposable <= 0) return null;
    if (budgetDisposable < totalMinPayment)
      return { shortfall: true, gap: totalMinPayment - budgetDisposable };
  ...
  const sortedDebts = useMemo(() => {
    ...
    return strategy === 'avalanche'
      ? c.sort((a, b) => Number(b.interestRate) - Number(a.interestRate))
      : c.sort((a, b) => Number(a.balance) - Number(b.balance));
  ...
  const sortedDebtsView = useMemo(() => {
    ...
    switch (debtSort) {
      ...
      case 'balance_desc':
        return c.sort((a, b) => Number(b.balance) - Number(a.balance));
      case 'balance_asc':
        return c.sort((a, b) => Number(a.balance) - Number(b.balance));
      case 'apr_desc':
        return c.sort(
      ...
      case 'apr_asc':
        return c.sort(
          (a, b) => Number(a.interestRate) - Number(b.interestRate)
        );
      case 'payment_desc':
        return c.sort(
          (a, b) => Number(b.minimumPayment) - Number(a.minimumPayment)
        );
      case 'payment_asc':
        return c.sort(
          (a, b) => Number(a.minimumPayment) - Number(b.minimumPayment)
        );
      case 'name':
  ...
  const upcomingPayments = useMemo(
    () =>
      activeDebts
        .filter((d) => d.paymentDueDay)
        .map((d) => ({ ...d, due: nextDueDate(d.paymentDueDay) }))
        .filter((d) => d.due && d.due.daysUntil <= 7)
        .sort((a, b) => a.due.daysUntil - b.due.daysUntil),
    [activeDebts]
  );
  function upcomingRateChanges() {
    const today = new Date().toISOString().split('T')[0];
    return activeDebts
      .flatMap((d) =>
        (d.rateChanges || [])
          .filter((rc) => !rc.applied && rc.date > today)
          .map((rc) => ({ ...rc, debtName: d.name }))
      )
    ...
    return (
      {
        ...
        Mortgage: '#fbbf24',
        Overdraft: '#f43f5e',
        'Buy Now Pay Later': '#e879f9',
        'Business Loan': '#38bdf8',
        'Medical Debt': '#fb7185',
        Other: '#94a3b8',
      }[type] || '#94a3b8'
    );
  ...
  function debtProgress(debt) {
    if (debt.termMonths && debt.remainingMonths)
      return Math.min(
        100,
        ((debt.termMonths - debt.remainingMonths) / debt.termMonths) * 100
      );
    const orig = Number(debt.originalBalance || debt.balance);
    return orig > 0
      ? Math.min(100, ((orig - Number(debt.balance)) / orig) * 100)
      : 0;
  }
  function getNextStep() {
    if (activeDebts.length === 0) return null;
    if (debtFreeProjection?.infinite)
      return {
        icon: '⚠️',
        text: `${debtFreeProjection.problemDebts[0].name} won't clear at current payments. Tap to fix it.`,
        action: () => {
          setShowPayoffIssues(true);
        },
      };
    if (priorityDebt)
      return {
        icon: '⚡',
        text: `Focus extra payments on ${priorityDebt.name} (${
  ...
  async function handleAddOrEdit() {
    ...
    try {
      ...
      const minimumPayment = form.useAutoMin
        ? calcMinPayment(balance, interestRate, form.type, remainingMonths)
        : Number(form.customMinPayment);
      if (editingId) {
        const existing =
          debts.find((d) => String(d.id) === String(editingId)) || {};
        const updated = {
          ...existing,
          name: form.name,
          type: form.type,
          interestRate,
          balance,
          useAutoMin: form.useAutoMin,
          minimumPayment,
          remainingMonths,
          rateChanges: form.rateChanges || [],
          paymentDueDay: form.paymentDueDay ? Number(form.paymentDueDay) : null,
          payoffGoalMonths: form.payoffGoalMonths
            ? Number(form.payoffGoalMonths)
            : null,
        };
        setDebts((prev) =>
          prev.map((d) => (String(d.id) === String(editingId) ? updated : d))
        );
        const { error } = await supabase
          .from('debts')
          .update(mapToDb(updated, session?.user?.id))
          .eq('id', editingId);
        ...
        const { error } = await supabase
          .from('debts')
          .insert([mapToDb(newDebt, session?.user?.id)]);
        if (error) {
          setDbError('Failed to add debt: ' + error.message);
  ...
  function handleEdit(debt) {
    setForm({
      ...
      totalTermMonths: debt.remainingMonths || '',
      useAutoMin: debt.useAutoMin !== false,
      customMinPayment: debt.minimumPayment,
      paymentDueDay: debt.paymentDueDay || '',
      payoffGoalMonths: debt.payoffGoalMonths || '',
  ...
  async function handlePayment() {
    ...
    if (!amt || amt <= 0) return;
    const debt = debts.find((d) => d.id === paymentModal.id);
    if (!debt) return;
    ...
    const newMin =
      debt.useAutoMin !== false
        ? calcMinPayment(newBalance, debt.interestRate, debt.type, newRemaining)
        : debt.minimumPayment;
    const updated = {
      ...debt,
      balance: newBalance,
    ...
    if (newBalance === 0) {
      setCelebration({
        ...
        subtitle: 'Amazing — one less debt to worry about.',
      });
    } else {
      setCelebration({
        emoji: '💪',
        title: 'Payment logged!',
        subtitle: `${formatCurrency(newBalance)} remaining on ${debt.name}.`,
      });
    }
  }

  function addRateChange() {
    if (!rateChangeForm.date || !rateChangeForm.newRate) return;
    setForm((f) => ({
      ...f,
      rateChanges: [...f.rateChanges, { ...rateChangeForm, applied: false }],
    }));
    setRateChangeForm({ date: '', newRate: '', label: '' });
    setShowRateChange(false);
  }
  function removeRateChange(i) {
    setForm((f) => ({
      ...f,
      rateChanges: f.rateChanges.filter((_, idx) => idx !== i),
    }));
  }
  ...
  async function exportData(format) {
    if (format === 'json') {
      const blob = new Blob(
        [
          JSON.stringify(
            { debts, strategy, exportedAt: new Date().toISOString() },
            null,
            2
          ),
        ],
      ...
      const rows = [
        ['Name', 'Type', 'Balance', 'APR %', 'Min Payment'],
        ...debts.map((d) => [
          d.name,
          d.type,
          d.balance,
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `settled-${new Date().toISOString().split('T')[0]}.csv`;
      a.click();
      URL.revokeObjectURL(url);
    }
  }
    ...
    try {
      ...
      if (data.debts && Array.isArray(data.debts)) {
        for (const debt of data.debts)
          await supabase
            .from('debts')
            .insert([mapToDb(debt, session?.user?.id)]);
        await loadData();
      }
    } catch {
      setDbError('Failed to restore backup.');
    }
  }
  ...
  const displayDebt =
    activeDebts.find((d) => String(d.id) === selId) || priorityDebt;
  const firstDebtDate =
    debts.length > 0
      ? debts.reduce(
          (earliest, d) =>
            !earliest || d.startDate < earliest ? d.startDate : earliest,
          null
        )
      : null;
  ...
  const S = {
    ...
    header: {
      background: 'linear-gradient(135deg,#12122a 0%,#0d0d17 100%)',
    ...
    tab: (a) => ({
      ...
      fontSize: 12,
      fontWeight: 600,
      background: a ? '#1e1e3a' : 'transparent',
      color: a ? '#fff' : '#64748b',
      transition: 'all 0.2s',
    }),
    ...
    btn: (v) => ({
      padding: v === 'sm' ? '7px 14px' : '12px 22px',
      borderRadius: 10,
      border: 'none',
      cursor: 'pointer',
      fontWeight: 600,
      ...
      color:
        v === 'primary'
          ...
          ? '#6366f1'
          : '#cbd5e1',
      border: v === 'outline' ? '1px solid #6366f1' : 'none',
      transition: 'all 0.2s',
    }),
    fab: {
      position: 'fixed',
    ...
    lockBanner: {
      background: 'linear-gradient(135deg,#6366f115,#a78bfa12)',
  ...
  return (
    <div style={S.app}>
      {showFreedom && (
        <FreedomScreen
          totalPaid={totalPaid}
          startDate={firstDebtDate}
          onDismiss={() => {
            setShowFreedom(false);
            setFreedomDismissed(true);
          }}
        />
      )}
      {celebration && (
          ...
          >
            <div style={{ fontSize: 12, color: '#64748b' }}>
              ...
              <strong
                style={{
                  color:
                    activeDebts.length >= FREE_LIMIT ? '#f97316' : '#e2e8f0',
                }}
              >
                {activeDebts.length}/{FREE_LIMIT}
              </strong>{' '}
              debts used
            </div>
            <button
          ...
          {activeDebts.length === 0 && completedDebts.length === 0 ? (
            <div style={{ paddingTop: 24 }}>
              <div style={{ textAlign: 'center', marginBottom: 28 }}>
                ...
                >
                  Add your first debt and instantly see when you'll be free, how
                  much interest you'll save, and exactly what to pay each month.
                </div>
              </div>
              <div
                style={{
              ...
              >
                ...
                <div
                  style={{ fontSize: 12, color: '#475569', marginBottom: 16 }}
                >
                  £6,200 paid · 42% complete
                </div>
                <div
                  style={{
                    background: 'linear-gradient(135deg,#34d39912,#a78bfa12)',
                    border: '1px solid #34d39940',
                    borderRadius: 14,
                    padding: 14,
                  }}
            ...
            <>
              {upcomingPayments.length > 0 && (
                ...
                >
                  ...
                  {upcomingPayments.map((d, i) => (
                    ...
                    >
                      <span>
                        <strong style={{ color: '#e2e8f0' }}>{d.name}</strong> ·
                        due {d.due.label}
                      </span>
                      <span
                        style={{
              ...
              {debtFreeProjection && !debtFreeProjection.infinite && (
                ...
                >
                  ...
                  >
                    {debtFreeProjection.words}
                  </div>
                  <div style={{ fontSize: 13, color: '#64748b' }}>
                    by {debtFreeProjection.date} · at minimum payments
                  </div>
                </div>
              )}
              ...
              {nextStep && (
                ...
                >
                  ...
                  <div
                    style={{ fontSize: 13, color: '#e2e8f0', lineHeight: 1.5 }}
                  >
                    <span style={{ marginRight: 6 }}>{nextStep.icon}</span>
                    {nextStep.text}
                  </div>
                </div>
              )}
              ...
              {debtFreeProjection?.infinite && (
                <div style={{ marginBottom: 16 }}>
                  <button
                    onClick={() => setShowPayoffIssues((v) => !v)}
                    style={{
                      width: '100%',
                      background: '#f9731615',
                  ...
                  >
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      >
                        {debtFreeProjection.problemDebts.length} debt
                        {debtFreeProjection.problemDebts.length !== 1
                          ? 's'
                          : ''}{' '}
                        won't clear at current payments
                      </div>
                      <div
                    ...
                    <span
                      style={{
                        fontSize: 14,
                        color: '#64748b',
                        transform: showPayoffIssues ? 'rotate(180deg)' : 'none',
                        ...
                        {debtFreeProjection.problemDebts.map((d, i) => (
                          <div
                            ...
                            style={{
                              ...
                              padding: 14,
                              marginBottom:
                                i < debtFreeProjection.problemDebts.length - 1
                                  ? 10
                                  : 0,
                            }}
                          >
                            ...
                            >
                              ...
                              <div style={{ fontSize: 12, color: '#94a3b8' }}>
                                ...
                                <strong style={{ color: '#34d399' }}>
                                  {formatCurrency(d.recommended)}/mo
                                </strong>{' '}
                                to clear in {d.targetMonths} months
                              </div>
                            </div>
                            <button
                              style={{
                              ...
                              onClick={async () => {
                                const updated = {
                                  ...d,
                                  minimumPayment:
                                    Math.ceil(d.recommended * 100) / 100,
                                  useAutoMin: false,
                                };
                                await supabase
                                  .from('debts')
              ...
              >
                ...
                >
                  ...
                  <div style={{ fontSize: 11, color: '#64748b' }}>
                    {completedDebts.length > 0
                      ? `${completedDebts.length} paid off ✓`
                      : 'none cleared yet'}
                  </div>
                </div>
                <div
                ...
                >
                  ...
                  >
                    {activeDebts.length
                      ? Math.max(
                          ...activeDebts.map((d) => Number(d.interestRate))
                        ).toFixed(1) + '%'
                      : '—'}
                  </div>
                  <div style={{ fontSize: 11, color: '#64748b' }}>APR</div>
                </div>
                <div
                  style={{
                    background: '#12122a',
                ...
                >
                  ...
                  <div style={{ fontSize: 11, color: '#6366f1', marginTop: 3 }}>
                    {showTimeline ? '▴ hide' : '▾ see timeline'}
                  </div>
                </div>
                <div
                  style={{
                ...
                (() => {
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        Your monthly commitment drops as each debt clears
                      </div>
                      {timeline.map((m, i, arr) => {
                        ...
                        const barWidth =
                          arr[0].total > 0 ? (m.total / arr[0].total) * 100 : 0;
                        return (
                          <div
                            key={i}
                            style={{
              ...
              <div
                style={{
                  background: 'linear-gradient(135deg,#22d3ee12,#6366f112)',
              ...
              >
                <div
                  style={{
                    ...
                    alignItems: 'center',
                    justifyContent: 'space-between',
                    marginBottom: showBudgetInput ? 12 : 0,
                  }}
                >
                  <div>
                    <div
                    ...
                    {budgetIncome > 0 && !showBudgetInput ? (
                      <div>
                        <div
                          style={{
                            fontSize: 20,
                        ...
                        >
                          {budgetProjection && !budgetProjection.shortfall
                            ? `Free in ${monthsToWords(
                                budgetProjection.months
                              )}`
                            : 'Budget shortfall'}
                        </div>
                        <div
                        ...
                        >
                          {formatCurrency(budgetMonthlyExtra)}/mo surplus to
                          debt
                        </div>
                      </div>
                    ) : (
                      <div style={{ fontSize: 13, color: '#64748b' }}>
                        Enter your income to see how fast you can clear
                        everything
                      </div>
                    )}
                  </div>
                  ...
                  >
                    {showBudgetInput ? 'Done' : 'Set budget'}
                  </button>
                </div>
                {showBudgetInput && (
                  <div>
                    ...
                    <input
                      ...
                      value={budgetIncome || ''}
                      onChange={(e) =>
                        setBudgetIncome(Number(e.target.value) || 0)
                      }
                    />
                    <div
                      style={{
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setBudgetExpenses(Number(e.target.value) || 0)
                      }
                    />
                    {budgetIncome > 0 && (
                      <div
                        ...
                        >
                          <span
                            style={{
                              ...
                              fontWeight: 700,
                              color:
                                budgetMonthlyExtra > 0 ? '#22d3ee' : '#f43f5e',
                            }}
                          >
                            Available for debt
                          </span>
                          ...
                          >
                            {budgetMonthlyExtra > 0
                              ? formatCurrency(budgetMonthlyExtra) + '/mo'
                              : 'Shortfall'}
                          </span>
                        </div>
                      </div>
                    ...
                    <div style={S.card}>
                      ...
                      {activeDebts.length > 1 && (
                        <div style={{ marginBottom: 14 }}>
                          ...
                          <select
                            value={selId}
                            onChange={(e) =>
                              setDashboardPaymentDebt(e.target.value)
                            }
                            style={{
                              width: '100%',
                              background: '#0d0d17',
                      ...
                      <div style={S.row}>
                        <div>
                          ...
                          <div
                            style={{
                              fontSize: 11,
                              fontWeight: 600,
                              color: typeColor(displayDebt?.type),
                              textTransform: 'uppercase',
                            }}
                          >
                            {displayDebt?.type}
                        ...
                        <div style={{ textAlign: 'right' }}>
                          ...
                          >
                            {formatCurrency(displayDebt?.balance)}
                          </div>
                          <div style={{ fontSize: 12, color: '#64748b' }}>
                            {displayDebt?.interestRate}% APR
                          </div>
                        </div>
                      </div>
                      <div style={{ marginTop: 12 }}>
                        <ProgressBar
                          percent={debtProgress(displayDebt)}
                          color={typeColor(displayDebt?.type)}
                        />
                      </div>
                      <div
                      ...
                      >
                        Min payment:{' '}
                        {formatCurrency(displayDebt?.minimumPayment)}/mo
                      </div>
                      <button
                        style={{
                          ...S.btn('primary'),
                        ...
                        onClick={() => {
                          setPaymentModal(displayDebt);
                          setPaymentAmount('');
                        }}
                      >
                        Log Payment — {displayDebt?.name}
                      </button>
                    </div>
                  );
                })()}
      ...
      {activeTab === 'debts' && (
        <div style={S.section}>
            ...
            >
              <button
                style={{
                  ...
                  fontWeight: 600,
                  background:
                    strategy === 'avalanche' ? '#6366f1' : 'transparent',
                  color: strategy === 'avalanche' ? '#fff' : '#475569',
                  transition: 'all 0.2s',
                }}
              ...
              <button
                style={{
                  ...
                  fontWeight: 600,
                  background:
                    strategy === 'snowball' ? '#6366f1' : 'transparent',
                  color: strategy === 'snowball' ? '#fff' : '#475569',
                  transition: 'all 0.2s',
                }}
          ...
          {sortedDebtsView.length === 0 && completedDebts.length === 0 && (
            ...
            const goalPmt =
              isPremium && debt.payoffGoalMonths
                ? calcGoalPayment(
                    debt.balance,
                    debt.interestRate,
                    debt.payoffGoalMonths
                  )
                : null;
            const extra = goalPmt
              ? goalPmt - Number(debt.minimumPayment)
              : null;
            const due = debt.paymentDueDay
              ? nextDueDate(debt.paymentDueDay)
              : null;
            const pendingChanges = (debt.rateChanges || []).filter(
              (rc) => !rc.applied
            );
            return (
              <div
              ...
              >
                <div
                  style={{
                    ...
                    width: 4,
                    height: '100%',
                    background: typeColor(debt.type),
                ...
                <div style={{ paddingLeft: 8 }}>
                  <div style={S.row}>
                    ...
                    <div style={S.pill(typeColor(debt.type))}>
                  ...
                  <div style={{ marginBottom: 8 }}>
                    ...
                    >
                      <span>
                        ...
                        paid · {debtProgress(debt).toFixed(1)}%
                      </span>
                      {debt.remainingMonths && (
                        <span>{debt.remainingMonths} mo left</span>
                      )}
                    </div>
                  </div>
                  <div
                  ...
                  >
                    ...
                    {due && (
                      <span
                        style={{
                          marginLeft: 8,
                          color: due.daysUntil <= 3 ? '#f97316' : '#475569',
                          fontWeight: due.daysUntil <= 3 ? 700 : 400,
                        }}
                      >
                        · due {ordinal(debt.paymentDueDay)}
                        {due.daysUntil <= 7 && (
                          <span
                            style={{
                              color:
                  ...
                  {goalPmt && (
                    ...
                    >
                      ...
                      >
                        🎯 Clear by {dateInMonths(debt.payoffGoalMonths)}
                      </div>
                      <div
                        style={{
                          display: 'flex',
                  ...
                  {!isPremium && debt.payoffGoalMonths && (
                    ...
                    >
                      <span style={{ fontSize: 12, color: '#6366f1' }}>
                        🔒 Goal set — unlock to see payment breakdown
                      </span>
                      <button
                        style={{
                          background: '#6366f1',
                  ...
                  {pendingChanges.map((rc, j) => (
                    ...
                    >
                      ⏰ Rate → {rc.newRate}% on {formatDate(rc.date)}
                    </div>
                  ))}
                  {(() => {
                    const today = new Date();
                    ...
                    return (
                      ...
                      >
                        ✅ Minimum payment of{' '}
                        {formatCurrency(thisMonthAuto.amount)} auto-applied this
                        month
                      </div>
                    );
                  })()}
        ...
        <div style={S.section}>
          <div style={{ marginTop: 4, marginBottom: 20 }}>
            ...
            <div style={{ fontSize: 13, color: '#475569' }}>
              See what a little extra does to your debt-free date
            </div>
          </div>
          {activeDebts.length === 0 ? (
            <div style={S.emptyState}>
              <div style={{ fontSize: 40, marginBottom: 12 }}>⚡</div>
              <div style={{ fontSize: 16, fontWeight: 600, color: '#64748b' }}>
                Add debts to see your simulation
              </div>
            </div>
          ) : (
            <SliderSection
      ...
      <button
        style={S.fab}
        onClick={() => {
          if (!isPremium && activeDebts.length >= FREE_LIMIT) {
            setShowUpgrade(true);
            return;
          }
          setShowForm(true);
      ...
      >
        <span style={{ fontSize: 18 }}>
          {!isPremium && activeDebts.length >= FREE_LIMIT ? '🔒' : '+'}
        </span>
        {!isPremium && activeDebts.length >= FREE_LIMIT
          ? 'Unlock to add more'
          : 'Add Debt'}
          ...
          <div style={S.modalBox} onClick={(e) => e.stopPropagation()}>
            ...
            >
              <div
                ...
                onClick={() => {
                  window.location.href =
                    'https://buy.stripe.com/5kQ00jcfv39o84W1pn4ow01';
                }}
              >
                <div
                  style={{
              ...
              <div
                ...
                onClick={() => {
                  window.location.href =
                    'https://buy.stripe.com/5kQ9AT6Vb25k3OG6JH4ow00';
                }}
              >
                <div
                  style={{
                ...
                >
                  Saves £X vs monthly · yours forever
                </div>
                <button
                  style={{
                    width: '100%',
            ...
            >
              ...
              {[
                [
                  '⚡',
                  'Pay Faster slider',
                  'See your debt-free date with any extra amount',
                ],
                [
                  '🎯',
                  'Payoff goals',
                  'Set a clear-by date per debt with full breakdown',
                ],
                ['🔥', 'Strategy comparison', 'Avalanche vs snowball'],
                [
                  '💰',
                  'Budget Accelerator',
                  'Debt-free date based on your real budget',
                ],
                [
                  '✦',
                  'Unlimited debts',
                  'Track every loan, card, and liability',
                ],
                ['⬇', 'Backup & export', 'JSON and CSV anytime'],
              ].map(([icon, title, sub]) => (
                <div
                  key={title}
                  style={{
        ...
        <div style={S.modal} onClick={() => setShowForm(false)}>
          <div style={S.modalBox} onClick={(e) => e.stopPropagation()}>
            ...
            <input
              style={S.input}
              name="name"
              placeholder="e.g. Barclaycard, Car Loan"
              value={form.name}
              onChange={fc}
            />
            ...
            >
              ...
              <div>
                <label style={{ ...S.label, marginBottom: 6 }}>APR (%)</label>
                <input
                  style={{ ...S.input, marginBottom: 0 }}
                  name="interestRate"
                  type="number"
            ...
            <button
              style={{
                ...
                borderRadius: 10,
                padding: '10px 14px',
                color: form.showAdvanced ? '#a78bfa' : '#64748b',
                fontSize: 12,
                fontWeight: 600,
                cursor: 'pointer',
                marginBottom: form.showAdvanced ? 14 : 20,
                display: 'flex',
                alignItems: 'center',
                justifyContent: 'space-between',
              }}
                ...
                <div style={{ marginBottom: 14 }}>
                  <div
                    style={{
                      ...
                      borderRadius: 10,
                      padding: '10px 14px',
                      marginBottom: form.useAutoMin ? 0 : 10,
                      display: 'flex',
                      alignItems: 'center',
                      justifyContent: 'space-between',
                    }}
                  ...
                  {!form.useAutoMin && (
                    <div style={{ marginTop: 10 }}>
                      <label style={S.label}>Custom minimum (£/mo)</label>
                      <input
                        style={S.input}
                        name="customMinPayment"
                        type="number"
                        placeholder="0.00"
                        value={form.customMinPayment}
                        onChange={fc}
                      />
                    </div>
                  )}
                ...
                <div style={{ marginBottom: 14 }}>
                  ...
                  {isPremium ? (
                    <select
                      ...
                      name="payoffGoalMonths"
                      value={form.payoffGoalMonths}
                      onChange={fc}
                    >
                ...
                <div style={{ marginBottom: 16 }}>
                  ...
                  >
                    ...
                    <button
                      ...
                      onClick={() => setShowRateChange((r) => !r)}
                    >
                      + Add
                    </button>
                  </div>
                  {form.rateChanges.length === 0 && !showRateChange && (
                    <div
                      style={{
                        fontSize: 12,
                        color: '#334155',
                    ...
                    >
                      e.g. 0% promo ending, mortgage fix expiring
                    </div>
                  )}
                  {form.rateChanges.map((rc, i) => (
                    ...
                    >
                      <div>
                        ...
                        >
                          → {rc.newRate}% on {formatDate(rc.date)}
                        </div>
                        {rc.label && (
                          <div style={{ fontSize: 11, color: '#64748b' }}>
                            {rc.label}
                  ...
                  {showRateChange && (
                    ...
                    >
                      ...
                      <input
                        style={S.input}
                        placeholder="e.g. End of 0% promo"
                        value={rateChangeForm.label}
                        onChange={(e) =>
                          setRateChangeForm((f) => ({
                            ...f,
                      ...
                      <div style={{ display: 'flex', gap: 8 }}>
                        <button
                          ...
                          onClick={() => setShowRateChange(false)}
                        >
                          Cancel
                        </button>
                        <button
      ...
      {paymentModal && (
        <div style={S.modal} onClick={() => setPaymentModal(null)}>
          <div style={S.modalBox} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ fontSize: 13, color: '#475569', marginBottom: 20 }}>
              {paymentModal.name} · {formatCurrency(paymentModal.balance)}{' '}
              remaining
            </div>
            ...
            <input
              style={S.input}
              type="number"
              placeholder={`Suggested: £${Number(
                paymentModal.minimumPayment
              ).toFixed(2)}/mo`}
              value={paymentAmount}
            ...
            <div style={{ display: 'flex', gap: 8, marginBottom: 14 }}>
              {[
                [paymentModal.minimumPayment, 'Min'],
                [
                  isPremium && paymentModal.payoffGoalMonths
                    ? calcGoalPayment(
                        paymentModal.balance,
                        paymentModal.interestRate,
                        paymentModal.payoffGoalMonths
                      )
                    : (Number(paymentModal.minimumPayment) * 1.5).toFixed(0),
                  isPremium && paymentModal.payoffGoalMonths
                    ? '🎯 Goal'
                    : '1.5×',
                ],
                [paymentModal.balance, 'Full'],
              ].map(([v, label], i) => (
                <button
                  key={i}
                  style={{ ...S.btn('sm'), flex: 1, fontSize: 11 }}
                  onClick={() => setPaymentAmount(String(Number(v).toFixed(2)))}
                >
                  {label}
                  <br />£{Number(v).toFixed(0)}
                </button>
              ))}
            </div>
            <div style={{ fontSize: 11, color: '#475569', marginBottom: 16 }}>
              ...
              <strong style={{ color: '#e2e8f0' }}>
                {formatCurrency(
                  Math.max(
                    0,
                    Number(paymentModal.balance) - Number(paymentAmount || 0)
                  )
                )}
              </strong>
            </div>
            {(() => {
              const amt = Number(paymentAmount || 0),
                min = Number(paymentModal.minimumPayment || 0),
                extra = amt - min;
              ...
              const base = simulatePayoff(
                ...
                paymentModal.interestRate,
                min,
                paymentModal.rateChanges
              );
              const upgraded = simulatePayoff(
                paymentModal.balance,
                paymentModal.interestRate,
                amt,
                paymentModal.rateChanges
              );
              if (!isFinite(base.months) || !isFinite(upgraded.months))
                return null;
              const saved = base.interest - upgraded.interest,
                monthsSaved = base.months - upgraded.months;
